...
all_data = {
    "ANN": []
    # model_index, k_of, k_if, hu,     n_val_errors, Nval, val_error_rate, n_train_errors, Ntrain, train_error_rate
}
    ...
    for (k_if, (train_index, val_index)) in enumerate(inner_fold):
        ...
        for n_hidden_units in hidden_units_interval:
            ...
            model = lambda: torch.nn.Sequential(
                torch.nn.Linear(M, n_hidden_units).to(device),  # M features to n_hidden_units
                torch.nn.Tanh(),  # 1st transfer function,
                torch.nn.Linear(n_hidden_units, 1).to(device),  # n_hidden_units to 1 output neuron
            ...
            mse = (sum(se).type(torch.float) / len(y_test)).data.cpu().numpy()  # mean
            ...
            mse_train = (sum(se_train).type(torch.float) / len(y_train)).data.cpu().numpy()  # mean
